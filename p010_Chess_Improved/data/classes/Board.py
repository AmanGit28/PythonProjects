...
from data.classes.Squares import Squares
...
class Board:
	...
	def generate_squares(self):
		...
		for y in range(8):
			for x in range(8):
				output.append(
					Squares(x,  y, self.tile_width, self.tile_height)
				)
		return output
	...
	def is_in_checkmate(self, color):
		...
		# Iterate through each piece to check if any valid move can resolve the check.
		for piece in pieces:
			...
			for target_square in valid_moves:
				...
				target_piece = target_square.occupying_piece

				# Make the move temporarily.
				piece.pos = target_pos
				target_square.occupying_piece = piece
				self.get_square_from_pos(start_pos).occupying_piece = None

				# Check if the move resolves the check.
				if not self.is_in_check(color):
					# Undo the move and return False since this is not checkmate.
					piece.pos = start_pos
					target_square.occupying_piece = target_piece
					self.get_square_from_pos(start_pos).occupying_piece = piece
					return False
	...
	def is_insufficient_material(self):
		...
		if len(pieces) <= 2:
			return True  # King vs King or other insufficient combinations
		# Add specific conditions for King + Bishop/Knight vs King
		return False
